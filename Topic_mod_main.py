# Program to perform Topic-Handling for n Documents using LDA (Dictionary is the Entire Corpus)
...
# Iterate through every topic , append first 5 topics based on Sscore, and remove Numbers (which are the Probab. Dist of each word in the corpus) and any other symbol like spaces or '*'
for i in range(0,len(final)):
	for j in range(0,5):
...
with open('output_tm_main.json', 'w') as outfile:  
    for item in d2:
